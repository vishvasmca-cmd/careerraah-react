...
export default async function AssessmentEnglishPage({ searchParams }: { searchParams?: Promise<{ role?: string, name?: string }> }) {
    const params = await searchParams;
    const role = params?.role ?? 'student';
    const name = params?.name ?? (role === 'student' ? 'Student' : 'your child');
        ...
        <div className="relative isolate min-h-full">
            ...
            <div className="container mx-auto max-w-3xl py-8 px-4 md:px-6 lg:py-12 fade-in">
                <div className="mb-8 text-center space-y-4">
                    <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200">Global / English Medium</Badge>
                    ...
                    <p className="mt-3 text-muted-foreground md:text-xl">
                        Psychometric analysis for {role === 'parent' ? `your child, ${name}` : `you, ${name}`}.
                    </p>
                </div>
